...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if --[[ exports["tp-housing"]:imClosesToCloset() or ]] (IsNearShop(clothingShops) < 9.0) then
        if args[1] and args[2] then
            ...
            exports['mythic_notify']:DoHudText('error', 'You need to do something like /outfitadd 1 party <br/ > <br/ > 1 being the slot id, party is the name of your outfit.', 10000)
        end
    else
        exports['mythic_notify']:DoHudText('error', 'You are not near a wardrobe!', 10000)
    end
end, false)
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    if --[[ exports["tp-housing"]:imClosesToCloset() or ]] (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            ...
            --exports['progressBars']:startUI(2600, "Clothing")
            --Citizen.Wait(3000)
            TriggerEvent('betrayed_skin:outfits', 3, args[1])
            TriggerServerEvent('drp-framework:loadArmour')
        else
            exports['mythic_notify']:DoHudText('error', 'You need to do something like /outfituse 1 <br/ > <br/ > 1 being the slot id that you will have had previously saved.', 10000)
        end
    else
        exports['mythic_notify']:DoHudText('error', 'You are not near a wardrobe!', 10000)
    end
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if --[[ exports["tp-housing"]:imClosesToCloset() or ]] (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            ...
            exports['mythic_notify']:DoHudText('error', 'You need to do something like /removeoutfit 1 <br/ > <br/ > 1 being the slot id that you will have had previously saved.', 10000)
        end
    else
        exports['mythic_notify']:DoHudText('error', 'You are not near a wardrobe!', 10000)
    end
...
AddEventHandler('betrayed_skins:listOutfits', function(skincheck)
    exports['mythic_notify']:DoHudText('inform', 'Here u can use /outfitadd, /outfituse or /removeoutfit')
	for i = 1, #skincheck do
		exports['mythic_notify']:DoHudText("inform", skincheck[i].slot .. " | " .. skincheck[i].name, 10000)
	end
end)
...
--[[shopBlips = {
    ...
    -- {x = 198.4602355957,y = -1646.7690429688,z = 29.80321884155}, -- Firestation
    -- {  x = 299.29, y = -598.45, z = 43.29 } , -- PILLBOX
...
AddEventHandler("facewear:adjust",function(faceType,remove)
	local handcuffed = exports["ecrp-death"]:GetDeath()
