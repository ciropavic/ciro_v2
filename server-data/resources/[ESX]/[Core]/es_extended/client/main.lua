	...
	ESX.TriggerServerCallback('esx:admincommand', function(result)
		if result then
			...
			if DoesBlipExist(WaypointHandle) then
				...
				exports.mythic_notify:DoHudText('success', 'Telported!')
			else
        export.mythic_notify:DoHudText('error', 'No waypoint set')
			end
		end
	end)
end)
...
AddEventHandler('esx:spawnVehicle', function(vehicle)
  ...
  if not vehicle then model = `gt3rs` end
	...
	if IsModelInCdimage(model) then
      ...
      SetVehicleNumberPlateText(vehicle, vehicle) --plate text
      ...
      exports["onyxLocksystem"]:givePlayerKeys(plate)
		...
		exports['mythic_notify']:DoCustomHudText('error', 'Invalid vehicle', 10000)
	end
end)
