...
AddEventHandler("utk_fh:outcome", function(oc, arg)
    ...
    elseif not oc then
        exports["mythic_notify"]:DoHudText("error", arg)
    end
end)
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" or PlayerData.job.name == "bcso" then
        exports["mythic_notify"]:DoHudText("inform", "A bank's alarms are triggered!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
...
AddEventHandler("utk_fh:freezeDoors", function()
    ...
    Citizen.CreateThread(function()
        while true do
            if PlayerData.job.name == "police" or PlayerData.job.name == "bcso" and not dooruse then
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    --exports["mythic_notify"]:DoHudText("error", "VAULT DOOR WILL CLOSE IN 30 SECONDS!")
    Citizen.Wait(420000)
...
AddEventHandler("utk_fh:startheist", function(data, name)
    TriggerServerEvent("utk_fh:toggleDoor", name, false) -- ensure to lock the second door for the second, third, fourth... heist
    disableinput = true
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    startdstcheck = true
    currentname = name
    --exports["mythic_notify"]:DoHudText("error", "You have 2 minutes until the security system activation.")
                ...
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            exports["mythic_notify"]:DoHudText("error", "You don't have the Secure ID Card.")
                        end
                    end)
                end
            end
