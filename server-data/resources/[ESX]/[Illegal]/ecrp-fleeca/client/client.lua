...
AddEventHandler("norp-fleeca:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        TriggerEvent('ox_inventory:notify', {
            type = 'error',
            text = "A bank's alarms are triggered!"
        })
        if not DoesBlipExist(blip) then
            blip = AddBlipForCoord(Config.Banks[name].doors.startloc.x, Config.Banks[name].doors.startloc.y,
                Config.Banks[name].doors.startloc.z)
