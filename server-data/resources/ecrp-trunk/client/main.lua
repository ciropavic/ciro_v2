...
RegisterCommand('trunkgetin', function()
    ...
    if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and not inside and GetVehiclePedIsIn(player, false) == 0 then
        ...
        if lockStatus == 4 or lockStatus == 2 then
            ...
            exports['mythic_notify']:DoHudText('error', 'Trunk is locked')
        ...
        elseif GetVehicleDoorAngleRatio(vehicle, 5) ~= 0.0 then
            ...
            if OffSet > 0 then
                ...
                AttachEntityToEntity(player, vehicle, 0, -0.1, (d1["y"] + 0.85) + offsets[OffSet]["yoffset"],
                    (d2["z"] - 0.87) + offsets[OffSet]["zoffset"], 0, 0, 40.0, 1, 1, 1, 1, 1, 1)

            else

                AttachEntityToEntity(player, vehicle, 0, -0.1, d1["y"] + 0.85, d2["z"] - 0.87, 0, 0, 40.0, 1, 1, 1, 1,
                    1, 1)
            ...
            while inside do
                ...
                if DropPosition["x"] == 0.0 then
                    ...
                    DrawText3DTest(DropPosition["x"], DropPosition["y"], DropPosition["z"],
                        "[G] Open/Close | [F] Climb Out")
            ...
            -- TriggerEvent("notification","The trunk is closed.", 2)
            exports['mythic_notify']:DoHudText('error', 'Trunk is closed')
...
function VehicleInFront()
    ...
    local rayHandle = CastRayPointToPoint(pos.x, pos.y, pos.z, entityWorld.x, entityWorld.y, entityWorld.z, 10, player,
        0)
