...
function refreshPlayerWhitelisted()
	...
	for k,v in ipairs({'police', 'bcso'}) do
...
RegisterNetEvent('wf-alerts:clNotifyE')
AddEventHandler('wf-alerts:clNotifyE', function(pData)
	if pData ~= nil then
		local sendit = false
		for i=1, #pData.recipientList do
			if pData.recipientList[i] == ESX.PlayerData.job.name then sendit = true break end
		...
		if sendit then
			Citizen.Wait(1500)
			if not pData.length then pData.length = 6000 end
			pData.street = getStreetandZone(vector3(pData.coords.x, pData.coords.y, pData.coords.z))
			SendNUIMessage({action = 'display', info = pData, job = ESX.PlayerData.job.name, length = pData.length})
      TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3, '10-1314', 1.0)
			waypoint = vector2(pData.coords.x, pData.coords.y)
			createBlip(pData)
			Citizen.Wait(pData.length+2000)
			waypoint = nil
...
RegisterCommand('911', function(playerId, args, rawCommand)
	if not args[1] then exports['mythic_notify']:DoHudText('error', 'You must include a message with your 911 call') return end
	...
	exports['mythic_notify']:DoHudText('success', 'Your message has been sent to the authorities')
end, false)
...
AddEventHandler('ecrp:officerdowned', function(streetName)
    local data = {displayCode = '10-13B', description = 'Officer Down', isImportant = 1, recipientList = {'police', 'bcso', 'ambulance'}, length = '10000', 
		blipSprite = 303, blipColour = 1, blipScale = 1.7, info = ('%s %s'):format(firstname, lastname), infoM = 'far fa-dizzy'}
    local dispatchData = {dispatchData = data, caller = ('%s %s'):format(firstname, lastname), street = streetName, coords = playerCoords}
...
AddEventHandler('ecrp:officerdownedE', function(streetName)
    local data = {displayCode = '10-13A', description = 'Officer Down URGENT', isImportant = 1, recipientList = {'police', 'bcso', 'ambulance'}, length = '10000', 
		blipSprite = 303, blipColour = 1, blipScale = 1.7, info = ('%s %s'):format(firstname, lastname), infoM = 'far fa-dizzy'}
    local dispatchData = {dispatchData = data, caller = ('%s %s'):format(firstname, lastname), street = streetName, coords = playerCoords}
    TriggerServerEvent('wf-alerts:svNotifyE', dispatchData)
end)