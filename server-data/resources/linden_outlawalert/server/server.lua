...
local dispatchCodes = {
	...
	melee = { displayCode = '10-10', description = _U('melee'), isImportant = 0, recipientList = {'police', 'bcso'},
	blipSprite = 652, blipColour = 84, blipScale = 1.5 },

	officerdown = {displayCode = '10-13B', description = _U('officerdown'), isImportant = 1, recipientList = {'police', 'bcso', 'ambulance'},
	blipSprite = 653, blipColour = 84, blipScale = 1.5, infoM = 'fa-portrait'},

	officerdownA = {displayCode = '10-13A', description = _U('officerdown'), isImportant = 1, recipientList = {'police', 'bcso', 'ambulance'},
	blipSprite = 653, blipColour = 84, blipScale = 1.5, infoM = 'fa-portrait'},

	persondown = {displayCode = '10-52', description = _U('persondown'), isImportant = 0, recipientList = {'police', 'ambulance', 'bcso'},
	blipSprite = 153, blipColour = 84, blipScale = 1.5, infoM = 'fa-portrait'},

	autotheft = {displayCode = '10-16', description = _U('autotheft'), isImportant = 0, recipientList = {'police', 'bcso'},
	blipSprite = 651, blipColour = 84, blipScale = 1.5, infoM = 'fa-car', infoM2 = 'fa-palette' },

	speeding = {displayCode = '10-66', description = _U('speeding'), isImportant = 0, recipientList = {'police', 'bcso'},
	blipSprite = 650, blipColour = 84, blipScale = 1.5, infoM = 'fa-car', infoM2 = 'fa-palette' },

	shooting = { displayCode = '10-13', description = _U('shooting'), isImportant = 0, recipientList = {'police', 'bcso'},
	blipSprite = 648, blipColour = 84, blipScale = 1.5 },

	driveby = { displayCode = '10-13', description = _U('driveby'), isImportant = 0, recipientList = {'police', 'bcso'},
...
AddEventHandler('wf-alerts:svNotify', function(pData)
	if not Blacklisted(source) then
		...
		if pData.recipientList[1] == 'police' or 'bcso' then TriggerEvent('mdt:newCall', details, pData.caller, vector3(pData.coords.x, pData.coords.y, pData.coords.z), false) end
	end
end)

RegisterServerEvent('wf-alerts:svNotifyE')
AddEventHandler('wf-alerts:svNotifyE', function(pData)
	if not Blacklisted(source) then
		local dispatchData
		if pData.dispatchCode == 'officerdown' then
			pData.info = getTitle(source)
		end
		if pData.dispatchCode == 'officerdownA' then
			pData.info = getTitle(source)
		end
		if pData.dispatchCode == 'persondown' then
			pData.caller = getCaller(source)
		...
		pData.colour = dispatchData.blipColour
		pData.scale = dispatchData.blipScale
		TriggerClientEvent('wf-alerts:clNotifyE', -1, pData)
		local n = [[
...
AddEventHandler('wf-alerts:svNotify911', function(message, caller, coords)
	if message ~= nil then
		...
		pData.recipientList = {'police', 'ambulance', 'bcso'}
		pData.length = 13000
